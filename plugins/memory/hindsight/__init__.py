...
RETAIN_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "timestamp": {"type": "string", "description": "Alias for occurred_at."},
            "document_id": {
                "type": "string",
                "description": "Stable document ID for updating one logical memory topic.",
            },
            "metadata": {
                "type": "object",
                "additionalProperties": {"type": "string"},
                "description": "String provenance fields such as source, workspace, and files.",
            },
        },
        "required": ["content"],
    },
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # Optional workspace-owned bank routing.
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        self._workspace_bank_routing = bool(cfg.get("workspace_bank_routing", False))
    ...
    def system_prompt_block(self) -> str:
        ...
        state = f"Active{label}, budget: {self._budget}."
        return f"# Hindsight Memory\n{routing}\n{state}\n{_SYSTEM_PROMPT_TAILS[mode]}"
    ...
    def _tool_retain(self, args: dict) -> str:
        ...
        metadata = {
            str(key): str(value) for key, value in (args.get("metadata") or {}).items()
        } or None
        item = self._build_retain_kwargs(
            content, context=context, metadata=metadata, tags=args.get("tags"),
            occurred_at=args.get("occurred_at") or args.get("timestamp"),
        )
        logger.debug("Tool hindsight_retain: bank=%s, content_len=%d, context=%s",
                     self._bank_id, len(content), context)
        self._retain_batch(item, bank_id=self._bank_id, document_id=args.get("document_id"))
    ...
    def _list_banks(self) -> list[dict[str, str]]:
        response = self._run_hindsight_operation(lambda client: client.banks.list_banks())
    ...
    def _handle_bank_routing(self, args: dict) -> str:
        if not self._workspace_bank_routing:
            return json.dumps({"routing": "disabled", "bank_id": self._bank_id, "source": self._bank_source})
        ...
        try:
            ...
            if bank_id not in existing:
                ...
                self._run_hindsight_operation(lambda client: client.acreate_bank(bank_id=bank_id))
            ...
            self._bank_source = str(env_path)
            return json.dumps({
                "routing": "bound", "bank_id": bank_id,
                "workspace": self._agent_workspace, "persisted_to": str(env_path),
